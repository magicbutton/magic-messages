...
export function useGraph<T>(url: string, scopes: string[]) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const testResponse = await https<T>(token, "GET", url);
        ...
        try {
          ...
          const testResponse = await https<T>(response.accessToken, "GET", url);
          setdata(testResponse);
          settoken(token);
          magicbox.setAuthToken(token, "MSAL");
        } catch (error) {
